...
function resolveApiKey(cfg: OpenClawConfig | undefined, explicit?: string): string {
  ...
  if (!token) {
    throw new Error(
      "AGENTMAIL_API_KEY or channels.agentmail.apiKey/tokenFile is required for AgentMail sends",
    );
  }
  return token;
}
...
function normalizeRecipient(to: string): string {
  ...
  const normalized = trimmed.replace(/^(agentmail|email|mail):/i, "").trim();
...
async function resolveInboxId(
  ...
  if (agentmailCfg?.defaultInboxId) {
    return agentmailCfg.defaultInboxId;
  }
  ...
  if (!first?.inbox_id) {
    throw new Error(
      "No inbox found. Create one first or specify channels.agentmail.defaultInboxId in config.",
    );
  }
...
export async function sendMessageAgentMail(
  ...
  const apiKey = resolveApiKey(opts.cfg, opts.apiKey);
  const recipient = normalizeRecipient(to);
  const inboxId = await resolveInboxId(apiKey, opts.cfg, opts.inboxId);

  const sleep = (ms: number) => new Promise((resolve) => setTimeout(resolve, ms));

  const sendWithRetry = async <T>(fn: () => Promise<T>, label: string): Promise<T> => {
    let lastErr: unknown;
  ...
  // Send new message
  const result = await sendWithRetry(async () => {
    const res = await fetch(`${AGENTMAIL_API_BASE}/inboxes/${inboxId}/messages`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${apiKey}`,
        "Content-Type": "application/json",
